...
This document explains how Bloud works. It's written for contributors who want to understand the system before diving into code.

## What Bloud Does

Bloud is a home server OS that makes self-hosting accessible. You install apps with one click, and they automatically configure themselves - SSO, database connections, integrations between apps, all handled for you.

The magic happens through three core ideas:
...
Bloud generates a simple `apps.nix` file:
...
Each app's `module.nix` defines what `enable = true` means - usually creating a systemd service that runs a podman container.
...
Let's trace what happens when you install Miniflux:

**1. User clicks Install**

```
POST /api/apps/miniflux/install
```

**2. Planning**

```go
plan := graph.PlanInstall("miniflux")
// Miniflux needs:
//   - database: required → postgres (auto-select, only option)
// Plan result:
//   - AutoConfig: [{miniflux, database, postgres}]
//   - No user choices needed
```
...
```go
transaction := buildInstallTransaction("miniflux", userChoices)
// Result:
//   apps["postgres"] = {Enabled: true}
//   apps["miniflux"] = {Enabled: true, Integrations: {database: postgres}}
```
...
- `podman-postgres.service` (if not already running)
- `podman-miniflux.service` (depends on postgres)
...
postgres starts → becomes healthy
    │
    ▼
miniflux starts → connects to postgres
    │
    ▼
both running
```

**7. Configuration (Systemd Hooks)**

```
Level 0: postgres
  StaticConfig: (nothing needed)
  HealthCheck: pg_isready
  DynamicConfig: (nothing needed)

Level 1: miniflux
  StaticConfig: Write Traefik SSO redirect config
  HealthCheck: Wait for /healthcheck
  DynamicConfig: Set admin user theme via API
```

**8. Complete**

Miniflux is running at `/embed/miniflux`, connected to postgres, with SSO configured.
...
- [graph-configurator-system.md](design/graph-configurator-system.md) - Detailed configurator design
- [embedded-app-routing.md](embedded-app-routing.md) - How apps are served in iframes
- [authentication.md](design/authentication.md) - SSO and auth flows
- [production-architecture.md](design/production-architecture.md) - Production deployment
