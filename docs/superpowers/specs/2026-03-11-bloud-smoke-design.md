...
4. **Wait for Miniflux** — poll `GET /api/apps` until Miniflux status is `running` (up to 5 minutes, 10s intervals)
...
Thin wrapper around `fetch` with the session cookie jar. Methods:

- `waitForSetupReady(timeout)` — polls `/api/setup/status`
- `createUser(username, password)` — POSTs to `/api/setup/create-user`, stores cookie
- `installApp(name)` — POSTs to install endpoint
- `waitForAppRunning(name, timeout)` — polls app status
...
1. Call existing `cmdStartPVE(build=true, install=true)` — ISO build + VM deploy + health checks
