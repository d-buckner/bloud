  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf appCfg.enable (lib.mkMerge [
    {
      ...
      systemd.user.services = {
        ...
        # Create a {name}.service alias so other apps can depend on this app by
        # its short name (as generated by metadata.nix). Only for standard naming
        # (name == serviceName); apps with custom serviceNames handle this themselves.
        // lib.optionalAttrs (name == serviceName) {
          "${name}" = {
            description = "${name} readiness alias";
            after = [ "podman-${serviceName}.service" ];
            requires = [ "podman-${serviceName}.service" ];
            serviceConfig = {
              Type = "oneshot";
              RemainAfterExit = true;
