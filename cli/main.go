...
func printUsage() {
	...
	fmt.Println()
	fmt.Println("Dev (Lima VM):")
	fmt.Println("  dev             Build + deploy + run host-agent on Lima VM (Ctrl-C to stop)")
	fmt.Println("  start           Show dev environment quick-start instructions")
	fmt.Println("  stop            Stop host-agent running on Lima VM")
	fmt.Println("  status          Show Lima VM and host-agent status")
	fmt.Println("  services        Show app container status on Lima VM")
	fmt.Println("  logs            Stream host-agent logs from Lima VM")
	fmt.Println("  attach          Open a shell on the Lima VM")
