...
func printUsage() {
	...
	if isPVEMode() {
		...
		fmt.Println("  smoke [flags]               Build ISO + full install + Playwright visual regression")
		fmt.Println("    --apps <app1> <app2>      Run only specified app tests (default: all apps)")
		fmt.Println("    --skip-deploy             Skip ISO build and VM deploy, run tests against existing VM")
