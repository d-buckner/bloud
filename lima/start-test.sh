#!/usr/bin/env bash
# Start the bloud TEST environment inside the Lima test VM
#
# Uses tmux to manage two processes:
# 1. Go hot reload via rsync + watch loop (port 3001)
# 2. Vite - Svelte dev server with HMR (port 5174)
#
# Test ports (different from dev):
#   - 3001: Go API (dev: 3000)
#   - 5174: Vite (dev: 5173)
#   - 8081: Traefik (dev: 8080)
#

set -e

# PROJECT_DIR can be passed as first argument or via environment variable
PROJECT_DIR="${1:-${BLOUD_PROJECT_DIR:-/home/bloud.linux/bloud}}"
LOCAL_SRC="/tmp/bloud-test-src"
DATA_DIR="/home/bloud/.local/share/bloud"  # Same as dev - VMs are separate
LOCAL_NODE_MODULES="/tmp/bloud-test-node-modules"
TMUX_SESSION="bloud-test"

echo "=== Bloud TEST Environment ==="
echo ""
echo "Test ports: API=3001, Vite=5174, Traefik=8081"
echo ""

# Check if project is mounted
if [ ! -d "$PROJECT_DIR/nixos" ]; then
    echo "Error: Project directory not mounted at $PROJECT_DIR"
    echo "Run: ./bloud test vm-create"
    exit 1
fi

# Create data directories (separate from dev)
mkdir -p "$DATA_DIR/nix"
mkdir -p "$DATA_DIR/traefik/dynamic"

# Ensure podman network exists
echo "Ensuring podman network..."
podman network create apps-net 2>/dev/null || true

# Initial sync of source (full copy)
echo "Syncing source files..."
rm -rf "$LOCAL_SRC"
mkdir -p "$LOCAL_SRC"
cp -r "$PROJECT_DIR/services/host-agent" "$LOCAL_SRC/"
cp -r "$PROJECT_DIR/apps" "$LOCAL_SRC/"

# Fix go.mod replace paths for VM directory structure
sed -i 's|=> ../../apps|=> ../apps|g' "$LOCAL_SRC/host-agent/go.mod" 2>/dev/null || true
sed -i 's|=> ../services/host-agent|=> ../host-agent|g' "$LOCAL_SRC/apps/go.mod" 2>/dev/null || true

# Build host-agent binary FIRST before starting services
# This prevents race condition where services start before binary exists
AGENT_BIN="/tmp/host-agent-test"
echo "Building host-agent binary..."
cd "$LOCAL_SRC/host-agent"
export BLOUD_PORT=3001
export BLOUD_DATA_DIR="$DATA_DIR"
export BLOUD_APPS_DIR="$LOCAL_SRC/apps"
export BLOUD_NIX_CONFIG_DIR="$DATA_DIR/nix"
export BLOUD_FLAKE_PATH="$PROJECT_DIR"
export BLOUD_FLAKE_TARGET="vm-test"
export BLOUD_NIXOS_PATH="$PROJECT_DIR/nixos"
export BLOUD_SSO_BASE_URL="http://localhost:8081"
export BLOUD_SSO_AUTHENTIK_URL="http://localhost:8081"

if go build -o "$AGENT_BIN" ./cmd/host-agent 2>&1; then
    echo "Host-agent built successfully at $AGENT_BIN"
else
    echo "ERROR: Failed to build host-agent"
    exit 1
fi
cd - > /dev/null

# Restart any failed services now that host-agent binary exists
echo "Restarting podman services..."
systemctl --user reset-failed 2>/dev/null || true
systemctl --user restart bloud-apps.target 2>/dev/null || true

# Set up node_modules for Linux
WEB_DIR="$LOCAL_SRC/host-agent/web"
if [ ! -d "$LOCAL_NODE_MODULES/node_modules" ] || [ "$PROJECT_DIR/services/host-agent/web/package.json" -nt "$LOCAL_NODE_MODULES/.installed" ]; then
    echo "Installing npm dependencies for Linux..."
    mkdir -p "$LOCAL_NODE_MODULES"
    cd "$LOCAL_NODE_MODULES"
    cp "$WEB_DIR/package.json" .
    cp "$WEB_DIR/package-lock.json" . 2>/dev/null || true
    npm install --prefer-offline 2>&1 | tail -5
    touch "$LOCAL_NODE_MODULES/.installed"
fi

# Symlink node_modules into local src (remove existing first)
rm -rf "$WEB_DIR/node_modules"
ln -s "$LOCAL_NODE_MODULES/node_modules" "$WEB_DIR/node_modules"

# Create Go watch script with rsync - TEST PORT 3001
GO_WATCH="/tmp/go-watch-test.sh"
cat > "$GO_WATCH" << 'GO_EOF'
#!/usr/bin/env bash
# Go hot reload with file sync - TEST ENVIRONMENT
SRC_DIR="$1"
LOCAL_DIR="$2"
DATA_DIR="$3"
NIX_CONFIG_DIR="$4"

cd "$LOCAL_DIR/host-agent"
export BLOUD_PORT=3001  # TEST PORT
export BLOUD_DATA_DIR="$DATA_DIR"
export BLOUD_APPS_DIR="$LOCAL_DIR/apps"
export BLOUD_NIX_CONFIG_DIR="$NIX_CONFIG_DIR"
export BLOUD_FLAKE_PATH="$SRC_DIR"
export BLOUD_FLAKE_TARGET="vm-test"  # TEST FLAKE TARGET
export BLOUD_NIXOS_PATH="$SRC_DIR/nixos"
export BLOUD_SSO_BASE_URL="http://localhost:8081"  # TEST PORT
export BLOUD_SSO_AUTHENTIK_URL="http://localhost:8081"  # TEST PORT

BIN="/tmp/host-agent-test"
LAST_HASH=""
PID=""

cleanup() {
    [ -n "$PID" ] && kill "$PID" 2>/dev/null
    exit 0
}
trap cleanup SIGINT SIGTERM

sync_files() {
    # Sync Go files from 9p mount to local copy
    cp -r "$SRC_DIR/services/host-agent/cmd" "$LOCAL_DIR/host-agent/" 2>/dev/null
    cp -r "$SRC_DIR/services/host-agent/internal" "$LOCAL_DIR/host-agent/" 2>/dev/null
    cp -r "$SRC_DIR/services/host-agent/pkg" "$LOCAL_DIR/host-agent/" 2>/dev/null
    cp "$SRC_DIR/services/host-agent/go.mod" "$LOCAL_DIR/host-agent/" 2>/dev/null
    cp "$SRC_DIR/services/host-agent/go.sum" "$LOCAL_DIR/host-agent/" 2>/dev/null
    # Fix go.mod replace path for VM directory structure
    sed -i 's|=> ../../apps|=> ../apps|g' "$LOCAL_DIR/host-agent/go.mod" 2>/dev/null
    # Sync apps (Go configurators + metadata)
    cp -r "$SRC_DIR/apps/"* "$LOCAL_DIR/apps/" 2>/dev/null
    # Fix apps/go.mod replace path for VM directory structure
    sed -i 's|=> ../services/host-agent|=> ../host-agent|g' "$LOCAL_DIR/apps/go.mod" 2>/dev/null
}

build_and_run() {
    echo "[$(date +%H:%M:%S)] [TEST] Syncing files..."
    sync_files
    echo "[$(date +%H:%M:%S)] [TEST] Building..."
    if go build -o "$BIN" ./cmd/host-agent 2>&1; then
        echo "[$(date +%H:%M:%S)] [TEST] Build successful, starting server on port 3001..."
        [ -n "$PID" ] && kill "$PID" 2>/dev/null && sleep 1
        "$BIN" &
        PID=$!
        echo "[$(date +%H:%M:%S)] [TEST] Server started (PID: $PID)"
    else
        echo "[$(date +%H:%M:%S)] [TEST] Build failed!"
    fi
}

# Initial build
build_and_run

# Watch loop
echo "[$(date +%H:%M:%S)] [TEST] Watching for changes (polling every 2s)..."
while true; do
    sleep 2
    HASH=$(find "$SRC_DIR/services/host-agent" -name '*.go' -not -path '*/web/*' -exec stat -c '%Y' {} \; 2>/dev/null | sort | md5sum)
    if [ "$HASH" != "$LAST_HASH" ]; then
        LAST_HASH="$HASH"
        echo ""
        echo "[$(date +%H:%M:%S)] [TEST] Change detected!"
        build_and_run
    fi
done
GO_EOF
chmod +x "$GO_WATCH"

# Create vite wrapper - TEST PORT 5174
VITE_WRAPPER="/tmp/run-vite-test.sh"
cat > "$VITE_WRAPPER" << 'VITE_EOF'
#!/usr/bin/env bash
# Vite dev server - TEST ENVIRONMENT (port 5174)
SRC_DIR="$1"
LOCAL_DIR="$2"
WEB_DIR="$LOCAL_DIR/host-agent/web"
NODE_MODULES="/tmp/bloud-test-node-modules/node_modules"

cd "$WEB_DIR"

sync_web_files() {
    cp -r "$SRC_DIR/services/host-agent/web/src" "$WEB_DIR/" 2>/dev/null
    cp -r "$SRC_DIR/services/host-agent/web/static" "$WEB_DIR/" 2>/dev/null
    cp "$SRC_DIR/services/host-agent/web/vite.config.ts" "$WEB_DIR/" 2>/dev/null
    cp "$SRC_DIR/services/host-agent/web/svelte.config.js" "$WEB_DIR/" 2>/dev/null
    cp "$SRC_DIR/services/host-agent/web/tsconfig.json" "$WEB_DIR/" 2>/dev/null
}

start_vite() {
    node "$NODE_MODULES/vite/bin/vite.js" dev --port 5174 &
    VITE_PID=$!
    echo "[$(date +%H:%M:%S)] [TEST] Vite started on port 5174 (PID: $VITE_PID)"
}

LAST_HASH=""

# Initial sync
sync_web_files

# Start vite in background
start_vite

echo "[$(date +%H:%M:%S)] [TEST] Watching for web file changes..."

cleanup() {
    kill "$VITE_PID" 2>/dev/null
    exit 0
}
trap cleanup SIGINT SIGTERM

# Watch for changes
while true; do
    sleep 2
    HASH=$(find "$SRC_DIR/services/host-agent/web/src" "$SRC_DIR/services/host-agent/web/static" -type f -exec stat -c '%Y' {} \; 2>/dev/null | sort | md5sum)
    if [ "$HASH" != "$LAST_HASH" ]; then
        LAST_HASH="$HASH"
        echo "[$(date +%H:%M:%S)] [TEST] Web files changed, syncing..."
        sync_web_files
    fi
    # Check if vite is still running
    if ! kill -0 "$VITE_PID" 2>/dev/null; then
        echo "[$(date +%H:%M:%S)] [TEST] Vite stopped, restarting..."
        start_vite
    fi
done
VITE_EOF
chmod +x "$VITE_WRAPPER"

# Check if tmux session already exists
if tmux has-session -t "$TMUX_SESSION" 2>/dev/null; then
    echo "Test environment already running!"
    echo ""
    echo "To view: ./bloud test attach"
    echo "To stop: ./bloud test stop"
    exit 0
fi

echo "Starting test servers..."
echo ""

# Create tmux session with two panes
tmux new-session -d -s "$TMUX_SESSION" -n test

# First pane: Go hot reload on TEST PORT
tmux send-keys -t "$TMUX_SESSION:test" "$GO_WATCH '$PROJECT_DIR' '$LOCAL_SRC' '$DATA_DIR' '$DATA_DIR/nix'" Enter

# Split horizontally for second pane
tmux split-window -h -t "$TMUX_SESSION:test"

# Second pane: Vite dev server on TEST PORT
tmux send-keys -t "$TMUX_SESSION:test.1" "$VITE_WRAPPER '$PROJECT_DIR' '$LOCAL_SRC'" Enter

# Set equal pane sizes
tmux select-layout -t "$TMUX_SESSION:test" even-horizontal

echo "=== Test Environment Started ==="
echo ""
echo "TEST Services (with hot reload):"
echo "  Go API:  http://localhost:3001  (test port)"
echo "  Web UI:  http://localhost:5174  (test port)"
echo "  Traefik: http://localhost:8081  (test port)"
echo ""
echo "Commands:"
echo "  ./bloud test attach   - View tmux session (Ctrl-B D to detach)"
echo "  ./bloud test logs     - View server output"
echo "  ./bloud test stop     - Stop test servers"
echo "  ./bloud test status   - Check service status"
echo ""
