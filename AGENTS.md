...
| **8443** | **Traefik TLS entrypoint (websecure).** Serves the same routes over HTTPS with the Bloud local-CA leaf — the SSO issuer (`sso.<base>:8443`) and the TLS mirror of 8080. | end users (SSO), app containers |
...
6. **SSO strategies** are exactly: `native-oidc`, `ldap`, `forward-auth`, `none`
   ...
   AppFlowy: none). AppFlowy is the one `none` app that still wires Bloud
   SSO — inside its own GoTrue as a custom OIDC provider (routing stays
   `none` because the SPA's API calls need a GoTrue-minted JWT, not proxy
   headers). The configurator skips that wiring for local/loopback issuers
   (GoTrue rejects them) and local sign-up is the fallback. See
   `apps/appflowy/INTEGRATION.md`.
...
8. **Config precedence**: env var > `secrets.json` (generated by
   ...
   `BLOUD_PODMAN_SOCKET`, `BLOUD_PORT` (3000), `BLOUD_TRAEFIK_PORT` (8080),
   `BLOUD_TRAEFIK_TLS_PORT` (8443), `BLOUD_BASE_DOMAIN`,
...
13. **TLS: Bloud owns a local CA** (`<dataDir>/tls/`, generated once at
    bootstrap by `internal/tlsca`): CA + 10-year leaf (SANs: base domain,
    `*.<base>`, literal `sso.<base>`, loopback IPs) + trust bundle. Traefik
    serves every route on the `websecure` entrypoint (:8443) in addition to
    HTTP (:8080). The SSO issuer is `https://sso.<base>:<tls-port>`.
    Containers that fetch the issuer server-side mount the bundle and set
    `SSL_CERT_FILE`. The literal `sso.<base>` SAN exists because
    OpenSSL/NSS reject single-label-suffix wildcards (`*.localhost` does not
    match `sso.localhost`). Browser trust is optional (Advanced → Proceed
    works); `./bloud setup` fetches the CA to `.bloud/ca.crt` with a
    per-OS trust one-liner.
