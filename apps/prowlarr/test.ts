...
test.describe('prowlarr', () => {
  // Prowlarr uses forward-auth SSO
  ...
  test('completes full SSO login flow', async ({ page, openApp }) => {
    ...
    // After SSO, should see Prowlarr's interface
    await expect(async () => {
      // Prowlarr shows indexers list, add indexer, or status
      const hasIndexers = await frame.getByText(/indexers/i).first().isVisible().catch(() => false);
      const hasAddIndexer = await frame.getByRole('link', { name: /add indexer/i }).first().isVisible().catch(() => false);
      const hasSearch = await frame.getByText(/search/i).first().isVisible().catch(() => false);
      const hasNavbar = await frame.locator('.navbar, nav, [class*="Navbar"]').first().isVisible().catch(() => false);
      const hasStatus = await frame.getByText(/status|health/i).first().isVisible().catch(() => false);

      expect(
        hasIndexers || hasAddIndexer || hasSearch || hasNavbar || hasStatus,
        'Should see Prowlarr interface after SSO'
      ).toBe(true);
    }).toPass({ timeout: 20000 });

    expect(page.url()).toContain('/apps/prowlarr');
  });
