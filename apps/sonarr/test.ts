...
test.describe('sonarr', () => {
  ...
  test('completes full SSO login flow', async ({ page, openApp }) => {
    ...
    // After SSO, should see Sonarr's interface
    await expect(async () => {
      // Sonarr shows series list, add series button, or calendar
      const hasSeries = await frame.getByText(/series|library/i).first().isVisible().catch(() => false);
      const hasAddSeries = await frame.getByRole('link', { name: /add new|add series/i }).first().isVisible().catch(() => false);
      ...
      expect(
        hasSeries || hasAddSeries || hasCalendar || hasNavbar,
        'Should see Sonarr interface after SSO'
      ).toBe(true);
    }).toPass({ timeout: 20000 });

    expect(page.url()).toContain('/apps/sonarr');
  });
