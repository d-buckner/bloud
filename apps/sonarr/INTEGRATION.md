...
| `POST http://localhost:8081/api/v2/torrents/createCategory`: form `category=tv-sonarr` | Create the category the client stores. `409 Conflict` (already exists) is accepted as already-done. |
...
so Sonarr resolves the provider over the shared `apps-net` network.
...
qBittorrent later re-runs Sonarr's `PostStart` through the framework's existing
staleness path, which adds the client then.

### Port constant coupling

`qbittorrentAppID`/`qbittorrentPort` in `apps/sonarr/configurator.go` mirror the
...
| `apps/sonarr/configurator.go` | Directory creation, `config.xml` pre-seed, API verification, root folder, download-client wiring |
