...
func TestConfigurator_Name(t *testing.T) {
	c := NewConfigurator(7878)
	if got := c.Name(); got != "radarr" {
		t.Errorf("Name() = %q, want %q", got, "radarr")
	}
}
...
func TestConfigurator_PreStart(t *testing.T) {
	...
	state := &configurator.AppState{
		Name:          "radarr",
		DataPath:      filepath.Join(tmpDir, "radarr"),
...
func TestConfigurator_getAPIKey_missingFile(t *testing.T) {
	tmpDir := t.TempDir()
	c := NewConfigurator(7878)

	_, err := c.getAPIKey(tmpDir)
	if err == nil {
...
func TestConfigurator_getDownloadClients(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := &Configurator{Port: 7878, baseURL: server.URL}
...
func TestConfigurator_createDownloadClient(t *testing.T) {
	tests := []struct {
		name       string
		statusCode int
		wantErr    bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := c.createDownloadClient(context.Background(), "test-key", client)

			if (err != nil) != tt.wantErr {
				t.Errorf("createDownloadClient() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !tt.wantErr && receivedPayload["name"] != "Test Client" {
				t.Errorf("expected name=Test Client, got %v", receivedPayload["name"])
			}
		})
	}
}
...
func TestConfigurator_ensureDownloadClient(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "different client exists",
			integrations:    map[string][]string{"downloadClient": {"qbittorrent"}},
			existingClients: []downloadClient{{ID: 1, Name: "Other Client"}},
			wantCreate:      true,
		},
	}
...
func TestConfigurator_getRootFolders(t *testing.T) {
	tests := []struct {
		name       string
		response   []rootFolder
		statusCode int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := &Configurator{Port: 7878, baseURL: server.URL}
			folders, err := c.getRootFolders(context.Background(), "test-key")

			if (err != nil) != tt.wantErr {
				t.Errorf("getRootFolders() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if len(folders) != tt.wantCount {
				t.Errorf("getRootFolders() returned %d folders, want %d", len(folders), tt.wantCount)
			}
		})
	}
}
...
func TestConfigurator_ensureRootFolder(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "already exists",
			existingFolders: []rootFolder{{ID: 1, Path: "/movies"}},
			wantCreate:      false,
		},
		{
			name:            "needs creation",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCreate && createdPath != "/movies" {
				t.Errorf("createdPath = %q, want /movies", createdPath)
			}
		})
	}
}
...
func TestConfigurator_PostStart(t *testing.T) {
	...
	// Should have called download client and root folder APIs
