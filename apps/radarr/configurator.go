...
func NewConfigurator(port int, deps configurator.Deps) *Configurator {
	...
	// The probe leaves the Radarr container: it asks the host's published
...
// Radarr's client stores and then adds the client, and when the provider is
// gone it prunes the client it left behind. An absent provider is never an
// error: Radarr works without a download client, and a later reconciliation
